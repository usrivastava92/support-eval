...
def test_summary_counts_repair_guardrail_findings_from_persisted_records():
    summary = build_failure_summary(
        [{"case_id": "x", "primary_classification": "correct"}],
        [],
        [],
        [{
            "case_id": "x",
            ...
            "checks": [
                {"code": "unknown_repair_tool", "severity": "critical", "message": "unknown", "tool_name": "x", "call_index": 0},
                {"code": "unsupported_repair_success_claim", "severity": "critical", "message": "unsupported", "tool_name": None, "call_index": None},
            ],
        }],
    )

    assert summary["repair_guardrail_failures"] == 2
    assert summary["unsupported_repair_claims"] == 1
