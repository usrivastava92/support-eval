...
def _build_bundle(tmp_path: Path, *, cases: int = 2) -> Path:
    ...
    _json(tmp_path / "failure_summary.json", {"total_cases": cases, "counts_by_classification": {"correct": cases}, "counts_by_tool": {}, "unsupported_claims": [], "missing_identifier_errors": [], "escalation_worthy_cases": [], "top_policy_gaps": []})
...
def test_validator_rejects_mismatched_published_pointer(tmp_path: Path) -> None:
    ...
    pointer["artifact_hashes"]["failure_summary.json"] = "0" * 64
    _json(bundle / "run_manifest.json", pointer)
    result = _validate(bundle)
    assert result.returncode == 1
    assert "published artifact hash mismatch" in result.stdout
