...
It then performs policy analysis, per-case evaluation, deterministic guardrail checks, per-case repair generation, and repair review in that order.
...
Use `OPENAI_BASE_URL` or `DEEPSEEK_BASE_URL` only when the corresponding endpoint must be overridden, and use `--base-url` to override the selected provider for one command.
Use `OPENAI_MODEL` or `DEEPSEEK_MODEL` in `.env` only as documentation for local configuration, because the current CLI selects a model with `--model` or uses the provider default.
...
The root validator checks required artifacts, result schemas, per-case coverage, call coverage and hashes, input hashes, ordered stage completion, and the published run pointer.
...
`run_manifest.json` points to the completed workspace manifest and records hashes for the root-level published results.
