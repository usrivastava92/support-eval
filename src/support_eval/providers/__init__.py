...
def resolve_provider(root: Path, provider_name: str | None = None, model: str | None = None, base_url: str | None = None, capture_dir: Path | None = None) -> Provider:
    ...
    if selected == "openai":
        ...
        return OpenAIProvider(key, model or value("OPENAI_MODEL") or OPENAI_MODEL, base_url or value("OPENAI_BASE_URL") or "https://api.openai.com/v1")
    key = value("DEEPSEEK_API_KEY")
    if not key:
        raise ValueError("DEEPSEEK_API_KEY is required for provider deepseek")
    return DeepSeekProvider(key, model or value("DEEPSEEK_MODEL") or DEEPSEEK_MODEL, base_url or value("DEEPSEEK_BASE_URL") or "https://api.deepseek.com/v1")
