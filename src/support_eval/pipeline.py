...
from .guardrails import run_guardrails, run_repair_guardrails
...
def run_pipeline(
    ...
    *,
    ...
    def invoke(stage: str, case: dict[str, Any] | None, **extra: Any) -> dict[str, Any]:
        ...
        input_artifacts = {
            "policy_analysis": ["tool_specs.json", "policy.md"],
            "case_evaluation": ["tool_specs.json", "policy.md", "cases.jsonl"],
            "case_repair": ["tool_specs.json", "policy.md", "cases.jsonl", "case_evaluations.json", "guardrail_checks.json"],
            "repair_review": ["tool_specs.json", "policy.md", "cases.jsonl", "case_repairs.json", "guardrail_checks.json", "repair_guardrail_checks.json"],
        }[stage]
        log: dict[str, Any] = {
            "sequence": sequence,
            "timestamp": _now(),
    ...
    def repair_cases() -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        repair_guardrails = run_repair_guardrails(cases, specs, repairs)
        atomic_write_json(workspace / "repair_guardrail_checks.json", repair_guardrails)
        if any(record["has_blocking_findings"] for record in repair_guardrails):
            raise ValueError("repair guardrails found blocking findings")
        review_extras = [
            {
            ...
            for evaluation, guardrail, repair, repair_guardrail in zip(
                evaluations, guardrails, repairs, repair_guardrails, strict=True
            )
        ]
        # Reviews are a required substep of CASES_REPAIRED, not an independent
        # top-level stage.
        reviews = invoke_each("repair_review", cases, review_extras)
        return guardrails, repairs, repair_guardrails, reviews

    guardrails, repairs, repair_guardrails, reviews = enter("CASES_REPAIRED", repair_cases)
    summary = enter(
        "FAILURE_PATTERNS_AGGREGATED",
        lambda: build_failure_summary(evaluations, guardrails, repairs, repair_guardrails),
    )
    ...
    outputs = {
        ...
        "repair_guardrail_checks.json": _json(repair_guardrails),
