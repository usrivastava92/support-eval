...
JSON_OUTPUTS = ("policy_analysis.json", "case_evaluations.json", "guardrail_checks.json", "case_repairs.json", "repair_guardrail_checks.json", "repair_reviews.json", "failure_summary.json")
...
def _validate_checks(label: str, record: dict[str, Any], errors: list[str]) -> None:
    ...
    for index, check in enumerate(checks):
        ...
        if not isinstance(check, dict):
            ...
            continue
        _exact_record(check_label, check, {"code", "severity", "message", "tool_name", "call_index"}, errors)
        for field in ("code", "severity", "message"):
            if not isinstance(check.get(field), str) or not check[field]:
                errors.append(f"{check_label} has invalid {field}")
        if check.get("tool_name") is not None and (not isinstance(check["tool_name"], str) or not check["tool_name"]):
            errors.append(f"{check_label} has invalid tool_name")
        if check.get("call_index") is not None and (not isinstance(check["call_index"], int) or isinstance(check["call_index"], bool) or check["call_index"] < 0):
            errors.append(f"{check_label} has invalid call_index")


def _validate_shapes(data: dict[str, Any], evaluations: dict[str, dict[str, Any]], guardrails: dict[str, dict[str, Any]], repairs: dict[str, dict[str, Any]], repair_guardrails: dict[str, dict[str, Any]], errors: list[str]) -> None:
    ...
    for label, records, fields in (
        ...
        ("repair_guardrail_checks", repair_guardrails, {"case_id", "checks", "has_blocking_findings"}),
    ):
        for case_id, record in records.items():
            _exact_record(f"{label}[{case_id}]", record, fields, errors)
    for case_id, record in guardrails.items():
        _validate_checks(f"guardrail_checks[{case_id}]", record, errors)
    for case_id, record in repair_guardrails.items():
        _validate_checks(f"repair_guardrail_checks[{case_id}]", record, errors)
        has_blocking_findings = record.get("has_blocking_findings")
        if not isinstance(has_blocking_findings, bool):
            errors.append(f"repair_guardrail_checks[{case_id}] has invalid has_blocking_findings")
        elif has_blocking_findings != any(
            check.get("severity") in {"critical", "high"}
            for check in record.get("checks", [])
            if isinstance(check, dict)
        ):
            errors.append(f"repair_guardrail_checks[{case_id}] has inconsistent has_blocking_findings")
        elif has_blocking_findings:
            errors.append(f"repair_guardrail_checks[{case_id}] has blocking findings")
...
def validate(root: Path, run: Path | None = None) -> list[str]:
    ...
    repair_guardrails = _by_case("repair_guardrail_checks", data["repair_guardrail_checks.json"], case_ids, errors)
    _by_case("repair_reviews", data["repair_reviews.json"], case_ids, errors)
    _validate_shapes(data, evaluations, guardrails, repairs, repair_guardrails, errors)
    ...
    if not isinstance(summary, dict):
        ...
        repair_checks = [
            check
            for record in repair_guardrails.values()
            for check in record.get("checks", [])
            if isinstance(check, dict)
        ]
        repair_guardrail_failures = len(repair_checks)
        unsupported_repair_claims = sum(
            check.get("code") == "unsupported_repair_success_claim"
            for check in repair_checks
        )
        if not isinstance(summary.get("repair_guardrail_failures"), int) or isinstance(summary["repair_guardrail_failures"], bool) or summary["repair_guardrail_failures"] != repair_guardrail_failures:
            errors.append("failure_summary repair_guardrail_failures is inconsistent")
        if not isinstance(summary.get("unsupported_repair_claims"), int) or isinstance(summary["unsupported_repair_claims"], bool) or summary["unsupported_repair_claims"] != unsupported_repair_claims:
            errors.append("failure_summary unsupported_repair_claims is inconsistent")
